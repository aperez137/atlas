...
def do(exptime):
    ...
    store_path = '/data-one/final/block_better_'+exptime

    con = pymysql.connect(host='localhost', user='astro', passwd='astro', db='astro')
    cursor = con.cursor()
    sql = "select target_name, filename, period, type from vsx, curve where vsx.oid=curve.oid and exptime='"+exptime+".0 s' and filename<>'NOT SUPPORTED' and mission='TESS' order by type asc"
    print("\n", sql, "\n")
    cursor.execute(sql)
    ...
    for row in data:
        ...
        tmp = {'file': row[1], 'period': row[2], 'type': row[3]}
    ...
    for target in buckets.keys():
        ...
        for row in buckets[target]:
            ...
            if(size > 0):
                ...
                tmp = {'curve': curve, 'reduction': reduction, 'mean_err': mean_err, 'size': size, 'better': better, 'period': row['period'], 'type': row['type']}
                curves.append(tmp)
...
def main(args):
    ...
    if(len(args) >= 2):
        ...
        print("Please use python block_better.py [exptime]")
