...
def compare(podFile, lcFile, areaLimit, outPath):
    ...
    for key in pre.keys():
        ...
        for g in grupos:
            ...
            pgr['G_'+str(groupCounter)] = []

            for e in g:
                unit = getData(path=podFile, key=e)
                pgr['G_'+str(groupCounter)].append({'type': unit['type'], 'id':e})

            groupCounter += 1
        

        for s in sobrantes:
            unit = getData(path=podFile, key=s)
            nulos.append({'type': unit['type'], 'id':s})
    ...
    print('Unmatched Elements', "\t", len(nulos))
    print('Total Grouped Elements', "\t", total)
    print('Total Groups Created', "\t", len(pgr.keys()), "\n")
    ...
    sumaryDic['Total Groups Created'] = len(pgr.keys())
...
def main(args):
    ...
    if len(args) == 5:
        ...
        print('\tPlease use: python compare.py [Path to aligned light curves file] [Path to pod file] [Path to output directory] [Max Area]')
        print('\tExample: python compare.py /home/user/align_120.bin /home/user/star_120.pod /home/user/out/ 10')
